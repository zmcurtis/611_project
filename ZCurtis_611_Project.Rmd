...
# FIX ORDER OR VARS IN CORPLOTS!!!
```{r corplot}
# Correlation matrix
mycor <- cor(allwine[,-1])
dir.create("figures", showWarnings = FALSE)
png("figures/corplot1.png", width = 8, height = 6, units = "in", res = 150)
...
corrplot(mycor2, type = "upper", order = "hclust", tl.col = "black")
dev.off()

mycor3<-cor(red[,-1])
dir.create("figures", showWarnings = FALSE)
png("figures/corplotred.png", width = 8, height = 6, units = "in", res = 150)
corrplot(mycor3, type = "upper", order = "hclust", tl.col = "black")
dev.off()
```
In the overall dataset, qe can see here that alcohol and density are highly NEGATIVELY correlated, makes sense since the higher the alcohol content, the lower the density. 

Yet when we split by wine type, we see some slightly different correlations. In the white wine correlation plot, we also see that residual sugar and density are highly correlated, and that free sulfur dioxide and total sulfur dioxide and moderately positively correlated. 

In the red wine correlation plot, fixed acidity and pH are highly negatively correlated, which makes sense, but it is curious that this does not appear in the white wine data set. It could be that white wines have higher or more basic acidity, so this trend is not being detected. 
```{r}
ggplot(data=allwine, aes(x=fixed.acidity, y=pH, color=type))+
  geom_point(alpha=0.4)
ggplot(data=allwine, aes(x=fixed.acidity, fill=type))+
  geom_density(alpha=0.4) #white wines do seem to have lower fixed acidity, thus red wines have higher range in acidity
ggplot(data=allwine, aes(x=pH, fill=type))+
  geom_density(alpha=0.4) #red wines do have higher pH as well 
```
From these plots, we can see that red wine does seem to have higher fixed acidity and higher pH than white wines, which would be opposite of what we predicted. # EXPAND AND EXPLORE HERE 
